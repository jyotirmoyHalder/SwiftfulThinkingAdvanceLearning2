...
//    }
    ...
    enum MyCustomAlert: Error, LocalizedError, AppAlert {
        ...
        var subtitle: String? {
            switch self {
            case .noInternetConnection:
                return "Please Check your interenet connection"
            case .dataNotFound:
                return nil
            case .urlError:
                return "Pleae Connect with the internet."
                
            }
        }
        
        var buttons: AnyView {
            AnyView(getButtonForAlert)
        }
        
        @ViewBuilder var getButtonForAlert: some View {
            switch self {
