...
//
//  Created by jyotirmoy_halder on 16/11/25.
//
...
//            _value = .init(initialValue: newValue.capitalized)
        }
    }

    init(wrappedValue: String) {
        self.value = wrappedValue.capitalized
//        _value = .init(initialValue: wrappedValue.capitalized)
    }
}
...
struct FileManagerValues {
    ...
    let userprofile1 = FileManagerKeypath(key: "user_profile1", type: User.self)
}
    ...
    var body: some View {
        VStack(spacing: 40) {
            ...
            Button(userProfile?.name ?? "no value") {
                userProfile = User(name: "Nick Sorno", age: 99, isPremium: true)
            }
        }
...
struct SomeBindingView: View {
    ...
    var body: some View {
        Button(userProfile?.name ?? "no value") {
            userProfile = User(name: "Manik", age: 97, isPremium: true)
        }
    }
}
